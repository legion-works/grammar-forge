...
describe('resolveCommonSettings', () => {
    ...
    it('strips a trailing slash from the bridge URL', () => {
        expect(
            resolveCommonSettings({ bridgeUrl: 'http://localhost:8000/' }).common.bridgeUrl,
        ).toBe('http://localhost:8000')
    })
    it('rejects non-http(s) URLs and falls back to the default', () => {
        expect(resolveCommonSettings({ bridgeUrl: 'ftp://x' }).common.bridgeUrl).toBe(
            'http://localhost:8000',
        )
        expect(resolveCommonSettings({ bridgeUrl: 'not-a-url' }).common.bridgeUrl).toBe(
            'http://localhost:8000',
        )
    })
    it('passes through a valid http URL', () => {
        expect(
            resolveCommonSettings({ bridgeUrl: 'http://bridge.local:9000' }).common.bridgeUrl,
    ...
    it('falls back to the default for non-finite realtimeDelayMs', () => {
        ...
        expect(resolveCommonSettings({ realtimeDelayMs: Infinity }).common.realtimeDelayMs).toBe(
            500,
        )
    ...
    it('allowRemoteBridge is strict === true', () => {
        expect(resolveCommonSettings({ allowRemoteBridge: true }).common.allowRemoteBridge).toBe(
            true,
        )
